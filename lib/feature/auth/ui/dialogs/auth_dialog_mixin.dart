...
mixin class AuthDialogMixin {
  ...
  static const currentPassCodeTitle = Text(
  ...
  static EdgeInsets getPadding(BuildContext context) =>
      ...
      (ScreenSize(context) is ScreenSmall
          ...
          : const .symmetric(vertical: 32));

  static ScreenLockConfig getScreenLockConfig(BuildContext context) {
    final theme = Theme.of(context);
    return ScreenLockConfig(
      themeData: theme,
      backgroundColor: theme.colorScheme.surfaceTint.withValues(alpha: 0.8),
      textStyle: theme.textTheme.titleMedium,
      titleTextStyle: theme.textTheme.titleLarge,
      buttonStyle: ButtonStyle(
        foregroundColor: WidgetStatePropertyAll(theme.colorScheme.onSurface),
        shape: const WidgetStatePropertyAll(CircleBorder(side: .none)),
        side: const WidgetStatePropertyAll(.none),
      ),
    );
  }
}
